...
./deploy -e ocp_user='developer' \
         ...
         -e namespace_name='malware-nextgen-blue-dev' \
         -e image_namespace_name='hr-authentication-amp' \
         -e filter_tags='create-build-config, deploy'\
         -e include_tags='create-build-config, deploy' \
         -e apps_subdomain='apps.mng-staging.dso.ncps.us-cert.gov' \
         -e scm_url='https://github.dso.ncps.us-cert.gov/malware-nextgen-pam/PamProxyService.git' \
         -e scm_ref='openshift'